...
function buildToken(ruleName, token, emit) {
    ...
    if (token.literal) {
        ...
        return buildStringToken(ruleName, token, emit);
    }

    if (token.subexpression) {
        return buildSubExpressionToken(ruleName, token, emit);
    }

    if (token.ebnf) {
        return buildEBNFToken(ruleName, token, emit);
    }
